...
instance exists, start a new instance whose name includes the directory."
  ...
  (let ((cwd (abbreviate-file-name
              (if (buffer-file-name)
                  (file-name-directory (buffer-file-name))
                default-directory))))
    (ghostel-lafrenierejm-find-directory cwd)))
#+end_src
...
#+begin_src emacs-lisp :noweb-ref ghostel/config
(defun ghostel-lafrenierejm-name (title)
  (format "*ghostel %s*"
          (projectile-lafrenierejm-make-relative-to-root-dwim
           default-directory)))
#+end_src

Use the above function to generate the names of ghostel buffers.

#+begin_src emacs-lisp :noweb-ref ghostel/custom
(ghostel-buffer-name-function #'ghostel-lafrenierejm-name)
#+end_src
