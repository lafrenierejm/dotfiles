...
#+begin_src emacs-lisp :noweb-ref shell/general
("C-x z" #'shell-lafrenierejm-find-directory)
#+end_src
...
Map ~shell-lafrenierejm-here~ to =C-z=.
This imitates the behavior of unix terminal applications.
...
**** Open vterm in a Given Directory

Define a function to open a directory in vterm.

#+begin_src emacs-lisp :noweb-ref vterm/config
(defun vterm-lafrenierejm-find-directory (directory)
  "Open VTerm in DIRECTORY.

If a VTerm instance for the directory already exists, switch to it.  If no
such instance exists, start a new instance whose name includes the directory."
  (interactive "DOpen vterm in: ")
  (let* ((buffer-name
          (format "*vterm %s*"
...
**** Open the Current Directory in vterm

Define a function to open the current directory in vterm.

#+begin_src emacs-lisp :noweb-ref vterm/config
(defun vterm-lafrenierejm-here ()
  "Open vterm in the current working directory.

If a vterm instance for the directory already exists, switch to it. If no such
instance exists, start a new instance whose name includes the directory."
  (interactive)
  (let ((cwd (abbreviate-file-name
              (if (buffer-file-name)
                  (file-name-directory (buffer-file-name))
                default-directory))))
    (vterm-lafrenierejm-find-directory cwd)))
#+end_src
...
This is used to remap the characters that Claude Code uses for its spinner and response indicator.

#+begin_src emacs-lisp :noweb-ref eat/config
(defun eat/lafrenierejm/prettify-symbols ()
  "Map symbol prettifications for vterm buffers."
...
#+begin_src emacs-lisp :noweb-ref eat/gfhook
('eat-mode-hook #'eat/lafrenierejm/prettify-symbols)
('eat-mode-hook #'prettify-symbols-mode)
#+end_src

*** Unbind C-z

#+begin_src emacs-lisp :noweb-ref eat/config
(add-to-list 'eat-semi-char-non-bound-keys [?\C-z])
