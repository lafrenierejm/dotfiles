...
(use-package display-line-numbers
  ...
  :custom
  (display-line-numbers 'relative "Display lines relative to the current line.")
...
Enable absolute line numbers when entering insert mode.
#+begin_src emacs-lisp :noweb-ref display-line-numbers/ghook
('evil-insert-state-entry-hook #'display-line-numbers-absolute)
#+end_src

Re-enable relative line numbers when leaving insert mode.
#+begin_src emacs-lisp :noweb-ref display-line-numbers/ghook
('evil-insert-state-entry-hook #'display-line-numbers-absolute)
('evil-insert-state-exit-hook #'display-line-numbers-relative)
#+end_src
...
Define a function to generate the top-level mode-line formatting.
#+begin_src emacs-lisp :noweb-ref minibuffer-line/config
(defun minibuffer-line-format ()
  "Return a list describing the contents of the minibuffer."
  `((:eval
...
And enable line numbers for all Lisp major modes.
#+begin_src emacs-lisp :noweb-ref display-line-numbers/ghook
(lisp-major-mode-hooks nil)
#+end_src
...
Display line numbers for all \src_emacs-lisp{sh-mode} buffers.
#+begin_src emacs-lisp :noweb-ref display-line-numbers/ghook
('sh-mode-hook nil)
#+end_src
...
Enable \src_emacs-lisp{smart-dash-mode} for all \src_emacs-lisp{sh-mode} buffers.
...
Enable \src_emacs-lisp{display-line-numbers-mode} in Groovy buffers.
#+begin_src emacs-lisp :noweb-ref display-line-numbers/ghook
...
Enable \src_emacs-lisp{display-line-numbers-mode} in \src_emacs-lisp{rust-mode} buffers.
#+begin_src emacs-lisp :noweb-ref display-line-numbers/ghook
('rust-mode-hook nil)
#+end_src
...
(use-package dtrt-indent
  ...
  :custom
  (dtrt-indent-global-mode t "Enable the mode globally"))
#+end_src
...
Inform the bytecode compiler of the above functions.
#+begin_src emacs-lisp :noweb-ref exwm-randr/functions
exwm-randr-assign-workspaces
#+end_src
