...
The startup screen and scratch message add no value, so they are disabled.
#+begin_src emacs-lisp
(setq
...
The ~*scratch*~ buffer starts in ~lisp-interaction-mode~ by default.
Let's change that to the generic ~fundamental-mode~.
#+begin_src emacs-lisp
...
(use-package faces
  ...
  :functions faces/lafrenierejm-setup-fonts

  :ghook
  ('after-make-frame-functions #'faces/lafrenierejm-setup-fonts)

  :config
  (defun faces/lafrenierejm-setup-fonts (&optional _frame)
    "Configure Noto fonts. Safe to call from `after-make-frame-functions'."
    (when (member "Noto Sans" (font-family-list))
      (set-face-attribute 'default nil :font "Noto Sans Mono")
      (set-fontset-font "fontset-default" nil "Noto Sans Mono")
      (set-fontset-font "fontset-default" nil "Noto Sans Math" nil 'append)
      (set-fontset-font "fontset-default" nil "Noto Emoji" nil 'append)
      (set-fontset-font "fontset-default" nil "Noto Sans Symbols" nil 'append)
      (set-fontset-font "fontset-default" nil "Noto Sans Symbols 2" nil 'append)

      (set-fontset-font "fontset-default" 'symbol "Noto Emoji")
      (set-fontset-font "fontset-default" 'symbol "Noto Sans Symbols" nil 'append)
      (set-fontset-font "fontset-default" 'symbol "Noto Sans Symbols 2" nil 'append)
      ...
      (set-fontset-font "fontset-default" 'cjk-misc "Noto Sans CJK KR Regular")
  ...
  (faces/lafrenierejm-setup-fonts)
  <<faces/config>>)
#+end_src
...
** Rainbow Delimiters
Rainbow Delimiters provides a minor mode that highlights delimiters such as parentheses based on their depth.
The highlight color of each level is distinct.
The package's source is available at [[https://github.com/Fanael/rainbow-delimiters][github.com/Fanael/rainbow-delimiters]].
...
('text-mode-hook #'display-line-numbers-evil-insert-entry)
#+end_src
...
('text-mode-hook #'display-line-numbers-evil-insert-exit)
#+end_src

#+begin_src emacs-lisp :noweb-ref display-line-numbers/ghook
('prog-mode-hook #'display-line-numbers-relative)
('text-mode-hook #'display-line-numbers-relative)
#+end_src
...
(use-package indent-bars-ts
  ...
  :ghook
  ('editorconfig-after-apply-functions #'indent-bars-ts/lafrenierejm/set-indent)
  ...
  :config
  (defun indent-bars-ts/lafrenierejm/set-indent (props)
    "Sync indent-bars with the major mode's indent size."
    (when-let* ((mode-indent-vars
                 '((json-ts-mode . json-ts-indent-offset)
                   (toml-ts-mode . toml-ts-indent-offset)))
                (var (alist-get major-mode mode-indent-vars nil nil
                                #'provided-mode-derived-p)))
      (setq-local indent-bars-spacing-override (symbol-value var))
      (indent-bars-setup))))
#+end_src
...
(use-package modus-themes
  ...
  :ghook
  ('ns-system-appearance-change-functions #'modus-themes/apply-theme)

  ;; Inform the bytecode compiler of the following functions.
  :functions
  modus-themes/apply-theme
  modus-themes/dbus-appearance-changed

  :config
  (defun modus-themes/apply-theme (appearance)
    "Load theme, taking current system APPEARANCE into consideration."
    (pcase appearance
      ('light (load-theme 'modus-operandi t))
      ('dark (load-theme 'modus-vivendi t))))

  (defun modus-themes/dbus-appearance-changed (namespace key value)
    "React to freedesktop appearance NAMESPACE KEY VALUE changes via dbus."
...
(use-package treesit
  ...
  :custom
  (treesit-enabled-modes t "Use tree-sitter wherever possible")
  (treesit-font-lock-level 4 "Fontify everything"))
#+end_src
...
Do not attempt to automatically discover projects.
#+begin_src emacs-lisp
(projectile-auto-discover nil)
#+end_src

Do not automatically register known projects.
#+begin_src emacs-lisp
...
~projectile-files-with-string~ uses a hardcoded ~rg~ invocation that omits
~--hidden~, so ~projectile-replace~ silently skips hidden directories like
~.github/~ even when their contents are tracked in Git.
#+begin_src emacs-lisp
(setf (alist-get 'rg projectile-files-with-string-commands)
      "rg -lF --no-heading --color never --hidden --glob=!.git/ ")
#+end_src

*** Return Path Relative to Project Root

#+begin_src emacs-lisp :noweb-ref projectile/config
(defun projectile-lafrenierejm-make-relative-to-root-dwim (path)
  "Print PATH relative to its Projectile root, if any.

If PATH is in a project as determined by Projectile, return PATH's
path starting starting at the project's root.

If PATH is _not_ in a project, return the abbreviated form of PATH's
path."
  (s-chop-suffix
   "/"
   (if-let* (((fboundp #'projectile-project-name))
             ((fboundp #'projectile-project-root))
             (project-root (projectile-project-root path))
...
From the package's [[https://codeberg.org/ideasman42/emacs-buffer-name-relative][README]]:
#+begin_quote
A minimal minor-mode that sets project relative paths for buffer names with optional path abbreviation.
#+end_quote
...
OK-IF-ALREADY-EXISTS is passed directly to `rename-file'."
  ...
  (let* ((buffer (current-buffer--obj-to-buffer buffer))
    ...
    (setq
     new-buffer-name
     (rename-buffer (buffer-name-relative--root-path-calc-relative new-file-name) t))
...
overwriting an existing file at NEW-FILE-NAME.  Interactively, confirmation is
...
(defun current-buffer-yank-name (&optional BASENAME)
  "Place the name of the current buffer on the kill ring.

The default behavior is to use the current buffer's name verbatim.  If optional
argument BASENAME is set, treat the buffer name as a path as return only the
basename."
  (interactive (list current-prefix-arg))
  (let* ((name-verbatim (buffer-name (current-buffer)))
         (name (if BASENAME
                   (file-name-nondirectory name-verbatim)
                 name-verbatim)))
    (kill-new name)
...
#+begin_src emacs-lisp :noweb-ref eglot/ghook
('markdown-mode-hook #'eglot-ensure)
#+end_src

#+begin_src emacs-lisp :noweb-ref eglot/config
(add-to-list 'eglot-server-programs '((markdown-mode) "vale-ls"))
#+end_src
...
Bind =C-w= in insert mode to its readline binding.
#+begin_src emacs-lisp
(:states 'insert
    ...
    (let ((old-point (point))
      ...
      (if (and comment-start-skip
          ...
          (progn
            ...
            ;; Go to the start of the comment's delimiter.
...
(defun evil-back-to-text ()
  ...
  ;; Move backward for non-whitespace character.
...
~counsel-grep-base-command~ expects a string with two instances of ~%s~.
The first instance is replaced with the regex.
The second is replaced with the name of the current file.

#+begin_src emacs-lisp :noweb-ref counsel/custom
(counsel-grep-base-command
 (if-let* ((command (cl-some #'executable-find '("rg"))))
     (s-join " " (list command
                       "--smart-case"
                       "--max-columns=88"
                       "--max-columns-preview"
...
The new binding is harder to accidentally press and opens up a convenient binding for /Browse at Remote/'s command.
...
#+begin_src emacs-lisp
('dired-mode-hook #'diff-hl-dir-mode)
#+end_src
...
Do not attempt to add default forge bindings.
#+begin_src emacs-lisp
...
Include fetch suffixes in the pull menu.
#+begin_src emacs-lisp
...
Enable smart dashes in EditorConfig buffers.
#+begin_src emacs-lisp :noweb-ref smart-dash/ghook
('editorconfig-conf-mode-hook #'smart-dash-mode)
#+end_src
...
(use-package comint
  ...
  :custom
  (comint-prompt-read-only t "Prevent editing shell prompt output.")
...
(use-package shell
  ...
  (defun shell-lafrenierejm-buffer-name (dir)
    "Generate the name of a shell buffer whose working directory is DIR."
    (format
     "*shell %s*"
     (projectile-lafrenierejm-make-relative-to-root-dwim dir)))

  (defun shell-lafrenierejm-rename-buffer (&rest _)
    "Rename the current shell buffer to include the current directory."
    (when (eq major-mode 'shell-mode)
      (rename-buffer (shell-lafrenierejm-buffer-name default-directory) t)))

  (advice-add 'comint-osc-process-output :after #'shell-lafrenierejm-rename-buffer)

  <<shell/config>>)
#+end_src

**** Open in a Given Directory

Define a function to start a shell in a given.

#+begin_src emacs-lisp :noweb-ref shell/config
(defun shell-lafrenierejm-find-directory (directory)
  "Open shell in DIRECTORY.

If a shell instance for the directory already exists, switch to it.  If no
such instance exists, start a new instance whose name includes the directory."
  (interactive "DOpen shell in: ")
  (let* ((buffer-name (shell-lafrenierejm-buffer-name directory))
         (default-directory directory))
    (shell buffer-name)
    (switch-to-buffer buffer-name)))
#+end_src

**** Open the Current Directory in shell

Define a function to open the current directory in shell.

#+begin_src emacs-lisp :noweb-ref shell/config
(defun shell-lafrenierejm-here ()
  "Open shell in the current working directory.

If a shell instance for the directory already exists, switch to it. If no such
instance exists, start a new instance whose name includes the directory."
  ...
  (let ((cwd (abbreviate-file-name
              (if (buffer-file-name)
                ...
                default-directory))))
    (shell-lafrenierejm-find-directory cwd)))
#+end_src
...
**** Name Ghostel Buffers With Project-Aware Paths

First we define a function that does nothing but return a project-aware buffer name.

#+begin_src emacs-lisp :noweb-ref ghostel/config
(defun ghostel-lafrenierejm-name-by-directory (_title)
  "Return a project-aware ghostel buffer name."
  (format "*ghostel %s*"
          (projectile-lafrenierejm-make-relative-to-root-dwim
           default-directory)))
#+end_src

Next we create a wrapper that not only returns the generated name but also sets the buffer-local ~ghostel--buffer-identity~.
This is needed so that as a ghostel buffer's directory changes, ghostel's internal tracking of which buffer corresponds to which directory will also be updated.

#+begin_src emacs-lisp :noweb-ref ghostel/config
(defun ghostel-lafrenierejm-name--mutate (title)
  "Generate and set `ghostel--buffer-identity' to a project-aware directory name."
  (let ((name (ghostel-lafrenierejm-name-by-directory title)))
    (when (derived-mode-p 'ghostel-mode)
      (setq-local ghostel--buffer-identity name))
    name))
#+end_src

Point ghostel to this mutating function for updating the names of ghostel buffers.

#+begin_src emacs-lisp :noweb-ref ghostel/custom
(ghostel-buffer-name-function #'ghostel-lafrenierejm-name--mutate)
#+end_src

**** Open a Directory In Ghostel

Define an interactive function to open a given directory in ghostel.

#+begin_src emacs-lisp :noweb-ref ghostel/config
(defun ghostel-lafrenierejm-find-directory (directory)
  "Open DIRECTORY in ghostel.

If a ghostel instance for DIRECTORY already exists, switch to it.  If no
such instance exists, start a new instance for DIRECTORY."
  (interactive "DOpen ghostel in: ")
  (let* ((default-directory directory)
         (ghostel-buffer-name (ghostel-lafrenierejm-name-by-directory nil)))
    (ghostel)))
#+end_src
...
#+begin_src emacs-lisp :noweb-ref ghostel/general
("C-x z" #'ghostel-lafrenierejm-find-directory)
#+end_src

**** Open Current Directory In Ghostel

#+begin_src emacs-lisp :noweb-ref ghostel/config
(defun ghostel-lafrenierejm-here ()
  "Open ghostel in the current working directory.

If a ghostel instance for the directory already exists, switch to it. If no such
instance exists, start a new instance whose name includes the directory."
  (interactive)
  (ghostel-lafrenierejm-find-directory default-directory))
#+end_src

Bind the above function to =C-z=.
This imitates the common behavior of terminal applications, in which =C-z= suspends the current process and returns the user to the invoking shell.

#+begin_src emacs-lisp :noweb-ref ghostel/general
("C-z" #'ghostel-lafrenierejm-here)
#+end_src

**** Compilation Mode Integration

Per [[https://dakra.github.io/ghostel/#compile-mode]]:
...
Per [[https://dakra.github.io/ghostel/#compilation-mode]]:

#+begin_quote
~ghostel-comint-mode~ replaces comint's built-in ~ansi-color-process-output~ with a stream filter that runs every chunk of process output through libghostty-vt's VT parser.
In =M-x shell= (and any other comint-derived buffer - REPLs, etc.) output renders with the same SGR fidelity a real ghostel terminal would give it, plus OSC 8 hyperlinks and OSC 7 directory tracking.
#+end_quote
...
- visually --- and, if scrolling is required, physically --- navigate between two prompts.
...
(use-package conf-mode
  ...
  (defconst conf-mode/ssh-config-dir
    (rx string-start (literal (getenv "HOME"))
        "/.ssh/config.d/"
        (one-or-more (not "/"))
        string-end)
...
#+begin_src emacs-lisp :noweb-ref smart-dash/ghook
('terraform-mode-hook #'smart-dash-mode)
#+end_src
...
Define a variable to hold the binary for formatting.
#+begin_src emacs-lisp :noweb-ref reformatter/config
(defvar tf-format-binary "terraform"
  "The binary to use when running `tf-format' functions.")
#+end_src

#+begin_src emacs-lisp :noweb-ref reformatter/config
(reformatter-define
  tf-format
  :program tf-format-binary
  :args '("fmt" "-no-color" "-"))
#+end_src

#+begin_src emacs-lisp :noweb-ref reformatter/ghook
('terraform-mode-hook #'tf-format-on-save-mode)
#+end_src

**** /Eglot/

Enable Eglot in Go buffers.
#+begin_src emacs-lisp :noweb-ref eglot/ghook
...
#+begin_src emacs-lisp :noweb-ref hcl-mode/config
(add-to-list 'terragrunt-mode-hook #'terragrunt-format-on-save-mode)
#+end_src
...
(defun go-mode-lafrenierejm-set-tab-width ()
  "Set `tab-width' to 4."
  (setq tab-width 4))
#+end_src

#+begin_src emacs-lisp :noweb-ref go-mode/gfhook
('go-mode-hook #'go-mode-lafrenierejm-set-tab-width)
#+end_src
...
(use-package gorepl-mode
  ...
  :general
  (:keymaps 'go-mode-keymap
   "C-c C-p" #'gorepl-run
   "C-c C-c" #'gorepl-run-load-current-file))
#+end_src
...
Enable Eglot in GraphQL buffers.
#+begin_src emacs-lisp :noweb-ref eglot/ghook
('graphql-ts-mode-hook #'eglot-ensure)
#+end_src

#+begin_src emacs-lisp :noweb-ref eglot/config
(add-to-list 'eglot-server-programs '((graphql-ts-mode graphql-mode) "graphql-lsp" "server" "-m" "stream"))
#+end_src
...
(defun elisp-eval-last-sexp (&optional replace)
  "Evaluate the preceding sexp, optionally REPLACE it with its result."
  (interactive "P")
  (if (not replace)
...
#+begin_src emacs-lisp :noweb-ref bootstrapping
...
The package's [[https://github.com/purcell/package-lint][homepage]] describes /Package Lint/ as "[a] linting library for elisp package metadata".
...
('(python-mode-hook python-ts-mode-hook inferior-python-mode-hook) #'python-set-comment-inline-offset)
#+end_src
...
('(python-mode-hook python-ts-mode-hook) nil)
#+end_src

**** /Smart Dash/

#+begin_src emacs-lisp :noweb-ref smart-dash/ghook
('(python-mode-hook python-ts-mode-hook inferior-python-mode-hook) #'smart-dash-mode)
#+end_src
...
(reformatter-define
  ruff-isort
  :program "ruff"
  :args (list "check" "--silent" "--select" "I" "--fix" "--fix-only" "-"))
#+end_src
...
('(python-mode-hook python-ts-mode-hook) #'wucuo-start)
#+end_src
...
(use-package groovy-mode
  ...
  (("\\.gdsl\\'" . groovy-mode)
   ("\\.groovy\\'" . groovy-mode)
   ("\\.[Jj]enkinsfile\\'" . groovy-mode)
   ("/[Jj]enkins\\(?:file\\)?\\'" . groovy-mode))
...
#+begin_src emacs-lisp :noweb-ref smart-dash/ghook
('ruby-mode-hook #'smart-dash-mode)
('inf-ruby-mode-hook #'smart-dash-mode)
#+end_src
...
#+begin_src emacs-lisp :noweb-ref smart-dash/ghook
('rust-mode-hook #'smart-dash-mode)
#+end_src
...
(reformatter-define rustfmt
  :program "rustfmt"
  :args (list "--edition=2021" input-file)
  :stdin nil
  :stdout nil)
#+end_src
...
(use-package ron-mode
  ...
  :preface
  (defconst ron-mode/cosmic-config-pattern
    (rx string-start (literal (getenv "HOME")) "/.config/cosmic/"
        ;; match subdirectories `com.system76.*'.
        "com.system76." (one-or-more (not "/")) "/"
        ;; all files within the `com.system76' subdirectories.
        (one-or-more any)
        string-end)
    "COSMIC desktop environment config files.")

  :init
  (add-to-list 'auto-mode-alist (cons ron-mode/cosmic-config-pattern #'ron-mode)))
#+end_src
...
(use-package sql
  ...
  :mode
  ("\\.sqlite\\'" . sql-interactive-mode)
  ("\\.sqlite3\\'" . sql-interactive-mode)
...
**** Load SQLite Files in ~sql-interactive-mode~

Define a function to load the current file in ~sql-interactive-mode~.
#+begin_src emacs-lisp :noweb-ref/
(defun sql-lafrenierejm-find-file-hook ()
  (when-let* ((_ (derived-mode-p 'sql-interactive-mode))
              (buffer (current-buffer))
              (file-name (buffer-file-name))
              (file-directory (file-name-directory file-name)))
    (let* ((buffer-name (buffer-name buffer))
           (buffer (progn (kill-buffer buffer)
                          (generate-new-buffer buffer-name))))
      (set-buffer buffer)
      (cd file-directory)
      (make-local-variable 'sql-database)
      (setq sql-database file-name)
      (sql-sqlite buffer))))
#+end_src

Add the above function to ~find-file-hook~ which is run every time a file is loaded.
#+begin_src emacs-lisp :noweb-ref sql/gfhook
('find-file-hook #'sql-lafrenierejm-find-file-hook)
#+end_src
...
From the package's [[https://github.com/pekingduck/emacs-sqlite3-api][home page]]:
#+begin_quote
...
(use-package dtrt-indent
  ...
  :custom
  (dtrt-indent-global-modes '((not typescript-ts-mode) prog-mode text-mode)))
#+end_src
...
(use-package agent-shell
  ...
  (:keymaps 'agent-shell-diff-mode-map
   :states '(movement normal)
   "C-c C-y" #'agent-shell-diff-accept-all)

  :custom
  (agent-shell-activity-group-expand-by-default 'latest "Expand current activity")
  (agent-shell-header-style nil "Do not display header")
  (agent-shell-show-welcome-message nil "Do not display welcome message")
  (agent-shell-chat-mode-enabled nil "Use comint-based interface")
  ...
  (setq
   ...
   ;; Use interactive login for Claude.
...
Allow resizing frames by pixel.
...
Disable the title bar on Linux.
...
(use-package frames-only-mode
  ...
  :ghook
  ('server-after-make-frame-hook #'frames-only-mode)

  :general
  (:keymaps 'frames-only-mode-mode-map
   "s-o" #'delete-other-frames
   "s-x" #'delete-frame)
