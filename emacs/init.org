...
(use-package indent-bars-ts
  ...
  (defun indent-bars-ts/lafrenierejm/set-indent (props)
    "Sync `indent-bars-spacing' with the major mode's indent size."
    (setq-local
     indent-bars-spacing-override
     (cond
      ((derived-mode-p 'json-ts-mode)
       json-ts-indent-offset)
      ((derived-mode-p 'toml-ts-mode)
       toml-ts-indent-offset)))
    ;; Update the indent bars if an override was set.
    (when indent-bars-spacing-override
      (indent-bars-setup))))
#+end_src
