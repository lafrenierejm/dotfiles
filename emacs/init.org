...
(use-package mixed-pitch
  ...
  :config
  (defun mixed-pitch-mode-off ()
    "Disable `mixed-pitch-mode'."
    (mixed-pitch-mode -1)))
#+end_src
...
#+begin_src emacs-lisp
(use-package ghq
  :straight (:host github :repo "lafrenierejm/emacs-ghq"
...
From [[https://github.com/CeleritasCelery/emacs-native-shell-complete][the project's README]]:
#+begin_quote
This package provides the exact same completions in ~shell-mode~ as you get in a [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Terminal-emulator.html][terminal emulator]] with the =TAB= key.
This means that completions are always accurate and synchronized with your current directory and shell environment.
This also means that subshell completion works correctly.
This is in contrast to packages like [[https://github.com/szermatt/emacs-bash-completion][emacs-bash-completion]] that use a separate process to generate “best guess” completions.
#+end_quote
...
Add /Native Company/'s completion package as a backend for /Company/.
...
Copyright (c) 2022-07-16 Álvaro Ramírez https://xenodium.com/a-lifehack-for-your-shell/"
  ...
  (dwim-shell-command-on-marked-files
   "Extract" "atool --extract --explain '<<f>>'")
  :utils "atool")
#+end_src
...
Disable /Mixed-Pitch Mode/ in XML buffers.
#+begin_src emacs-lisp :noweb-ref mixed-pitch/ghook
('nxml-mode-hook #'mixed-pitch-mode-off)
#+end_src
...
Disable /Mixed-Pitch Mode/.
#+begin_src emacs-lisp :noweb-ref mixed-pitch/ghook
('yaml-mode-hook #'mixed-pitch-mode-off)
#+end_src
