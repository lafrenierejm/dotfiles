...
(use-package shell
  ...
  (defun shell-lafrenierejm-rename-buffer (&rest _)
    ...
    (when (eq major-mode 'shell-mode)
      (rename-buffer
       (format "*shell %s*" (projectile-lafrenierejm-make-relative-to-root-dwim
                             default-directory)))))
...
#+begin_src emacs-lisp :noweb-ref vterm/general
("C-x z" #'vterm-lafrenierejm-find-directory)
#+end_src
...
Map ~vterm-lafrenierejm-here~ to =C-z=.
