...
(defun current-buffer/yank-path (&optional project-relative)
  "Place the current buffer's file path, optionally PROJECT-RELATIVE, on the kill ring."
  (interactive (list current-prefix-arg))
  (when-let* ((filename (or (buffer-file-name) default-directory))
              (filepath-absolute (expand-file-name filename))
              (filepath (if project-relative
                            (car (projectile-make-relative-to-root (list filepath-absolute)))
                          filepath-absolute)))
    (kill-new filepath)
    (message "Copied buffer path '%s' to the clipboard." filepath)))
#+end_src
