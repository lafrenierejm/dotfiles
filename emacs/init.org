...
**** Do Not Source Interactive Shells

#+begin_src emacs-lisp :noweb-ref exec-path-from-shell/custom
(exec-path-from-shell-arguments '("-l") "Load as a login shell.")
#+end_src
...
\src_emacs-lisp{cl-some} is provided by the /CL Extra/ package.
...
#+begin_src emacs-lisp :noweb-ref evil-collection/general
...
Unbind keys to avoid shadowing global bindings.
#+begin_src emacs-lisp :noweb-ref evil-collection/general
(:keymaps 'magit-mode-map
 "/" nil
 "C-d" nil
...
(use-package lsp-python-ms
  ...
  ('hack-local-variables-hook
   (list #'lsp-python-ms-enable #'lsp-python-ms-set-executable))

  :custom
  (lsp-python-ms-auto-install-server
   (eq system-type 'darwin)
   "Automatically install the server when running macOS.")

  :config
  (defun lsp-python-ms-enable ()
    "Load and enable this package."
    (when (derived-mode-p 'python-mode)
      (require 'lsp-python-ms)
      (lsp)))
  (defun lsp-python-ms-set-executable ()
    "Set the Python executable using pyvenv."
    (when pyvenv-activate
      (setq lsp-python-ms-python-executable
            (expand-file-name "bin/python" pyvenv-activate)))))
#+end_src
...
*** Autformat via =xmllint=

#+begin_src emacs-lisp :noweb-ref reformatt/config
(reformatter-define
  xml-format
  :program "xmllint"
  :args '("--format" "-"))
...
('exwm-update-title-hook
 (defun exwm-update-title ()
   "Use web browsers' windows names as the windows' buffers names."
   (when-let ((_ (cl-some (lambda (name) (string-equal name exwm-class-name))
                          '(Firefox Icecat Nightly Nyxt))))
